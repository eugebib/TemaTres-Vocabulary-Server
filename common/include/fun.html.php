...
function HTMLlistaTerminosEstado($estado_id, $limite = "")
{
	...
	while ($array = $sql->FetchRow()) {
		...
		$rows.='<tr>
					<td>
						'.HTMLlinkTerm($array,array("style"=>$styleTerm,"modal"=>1)).'
					</td>
					<td>'.
						$array["cuando"].'
					</td>
...
function HTMLsugerirTermino($texto, $acumula_temas = "0")
{
	...
	if (count($sugerencias) > 0) {
		...
		foreach ($sugerencias as $sugerencia) {
			$rows.='<li>
						<a href="'.URL_BASE.'index.php?'.FORM_LABEL_buscar.'='.$sugerencia['string'].'&amp;sgs=off" title="'.LABEL_verDetalle.$sugerencia['string'].'">
							<em><strong>'.mb_strtolower($sugerencia['string'], "utf8").'</strong></em>
						</a>
					</li>';
		}
		$rows .= '</ul>';
