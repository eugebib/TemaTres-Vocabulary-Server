...
if ($_SESSION[$_SESSION["CFGURL"]]["ssuser_nivel"]>0) {
	...
	function wichReport($task)
	{
		...
		return sql2xls($sql,string2url($_SESSION["CFGTitulo"]).'.csv',$_GET["csv_encode"]);
	}
...
function HTMLListaUsers()
{
	...
	$rows.='<div class="table-responsive">
		        <table id="tableusers" class="table table-striped table-bordered table-condensed table-hover" summary="'.MENU_Usuarios.'">
		        	<thead>
		        		<tr>
		        			<th class="izq" colspan="4">'.
		        				MENU_Usuarios.' &middot; <a class="btn btn-primary btn-xs" href="admin.php?user_id=new" title="'.MENU_NuevoUsuario.'">'.MENU_NuevoUsuario.'</a>
		        			</th>
		        		</tr>
		        		<tr>
	...
	while ($listaUsers = $sqlListaUsers->FetchRow()) {
		...
		$rows.='	<tr>
						...
						<td class="izq">'.$listaUsers[orga].'</td>
						<td>'.$fecha_alta[dia].'-'.$fecha_alta[descMes].'-'.$fecha_alta[ano].' ('.arrayReplace(array("ACTIVO","BAJA"),array(LABEL_User_Habilitado,LABEL_User_NoHabilitado),$listaUsers[estado]). ')</td>';
	...
	$rows.='	</tbody>
				<tfoot>
					<tr>
						<td class="izq">'.ucfirst(LABEL_TotalUsuarios).'</td>
						<td colspan="3">'.SQLcount($sqlListaUsers).'</td>
					</tr>
				</tfoot>
			</table>
		</div>';
