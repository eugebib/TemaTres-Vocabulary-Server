...
function SQLbuscaExacta($texto)
{
	...
	tema.tema = $texto
	$where
	 and (relaciones.rel_rel_id is NULL or relaciones.rel_rel_id!=44)
	group by tema.tema_id
...
function SQLdirectTerms($tema_id)
{
	...
	return SQL("select","if(uf.tema_id is not null,0,if(bt.tema_id is not null,1,if(nt.tema_id is not null,2,3))) as rel_order,
		...
		rt.tema_id as rt_tema_id,rt.code as rt_code,rt.tema as rt_tema,trr.value_code as rr_code,rt.isMetaTerm as rt_isMetaTerm,
		r.*
...
function SQLsimiliar($texto,$lista_temas_id="0")
{
	...
	*/
	...
	$sql = SQL("select","
			t.tema,
			length(t.tema) as largo
		FROM
			$DBCFG[DBprefix]tema as t
		WHERE
			length(t.tema) between $minstrlen and $maxstrlen and
			t.estado_id ='13'
			$where
		ORDER BY
			tema, largo",
